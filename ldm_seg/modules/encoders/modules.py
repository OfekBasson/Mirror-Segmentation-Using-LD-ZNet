...
from ldm_seg.modules.x_transformer import Encoder, TransformerWrapper  # TODO: can we directly rely on lucidrains code and simply add this as a reuirement? --> test
...
class FrozenSDUNet(AbstractEncoder):
    ...
    def forward(self, z, c, timestep=None):
        with self.model.ema_scope():
            if timestep is None:
                t = torch.randint(self.timesteps[0], self.timesteps[-1], (z.shape[0],), device=z.device).long()
            else:
